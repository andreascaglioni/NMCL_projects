...
%   Detailed explanation goes here
    ...
    %compute slopes
    slope(:,1) = eval_slope(dp(:,1), dm(:,1), SlopeNumber); %N+2
    slope(:,2) = eval_slope(dp(:,2), dm(:,2), SlopeNumber);
...
%     rhs = -1/dx*(eval_flux(Xhm(2:N+1,:), Xhp(3:N+2,:), FluxNumber, f1, f2, g, maxVel) ...
%                 -eval_flux(Xhm(1:N,:), Xhp(2:N+1,:), FluxNumber, f1, f2, g, maxVel)) + SAver;
end