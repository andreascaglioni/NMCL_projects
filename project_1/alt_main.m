...
while time < T
    ...
    S1Int = (S1Prim(uh(2:end-1), xx(2:end)', time) - S1Prim(uh(2:end-1), xx(1:end-1)', time))/dx;
    S2Int = (S2Prim(uh(2:end-1), xx(2:end)', time) - S2Prim(uh(2:end-1), xx(1:end-1)', time))/dx;
    % time advancement
    ...
    plotSOlAtTIme(cc, hh, @(x)hExa(x,time), mh, @(x)mExa(x,time));
%     press = waitforbuttonpress;
    pause(0.001);
end
%% plot final solution
